...
def Requirement(direntry):
	...
	#return 'blow' in direntry.path.lower() and direntry.name.lower().endswith('pptx') 
	#return 'blow' in direntry.path.lower() and direntry.name.lower().endswith('fcstd') 
	#return direntry.name.lower().endswith('step')
	#return direntry.name.lower().endswith('py')
	...
	return direntry.name.lower().endswith('xbm')
...
def AddToSelection(baseDir, selection, requirement, level):
	...
	try:
		...
		print('Error encountered in %s' % baseDir)
...
files=[de.name for de in selection]

files=['Timer.xbm', 'Intensity.xbm', 'Color.xbm', 'Exit.xbm',
	'Timer03.xbm', 'Timer05.xbm', 'Timer10.xbm', 'Timer20.xbm',
	'Intensity025.xbm', 'Intensity035.xbm', 'Intensity050.xbm', 'Intensity070.xbm', 'Intensity100.xbm',
	'White.xbm', 'Yellow.xbm', 'Orange.xbm', 'Red.xbm',
	'ExitCancel.xbm', 'ExitConfirm.xbm']

result=[]

#result.append("//[%s]\n\n" % ",\n".join(["'%s'" % f for f in files])) 

# Overview of files and numbers
...
for i, f in enumerate(files):
	...
	result.append("\n    { // %d %s\n" % (i,f.replace('.xbm','')))
	result.extend(lines[3:-1]) 
	result.append(lines[-1].replace(';',',')) 

#result.extend([de.name for de in selection])
result=''.join(result)
