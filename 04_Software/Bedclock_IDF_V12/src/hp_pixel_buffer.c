...
const hp_bitmap_t HP_BITMAP_INIT = {.bitmap = NULL, .mask = NULL };
...
void hp_bitmap_init(hp_bitmap_t *bitmap, const uint16_t width, const uint16_t height) {
    ...
    bitmap->bitmap = malloc(hp_bitmap_buffersize(bitmap));

    // Initialize mask as NULL
    bitmap->mask = NULL;
}

void hp_bitmap_clear(hp_bitmap_t *bitmap) {
    for(uint16_t b=0; b<hp_bitmap_buffersize(bitmap); b++) bitmap->bitmap[b]=0;
}

void hp_bitmap_delete(hp_bitmap_t *bitmap) {
    free(bitmap->bitmap);
    if(bitmap->mask != NULL) free(bitmap->mask);
}

void hp_bitmap_copy(const hp_bitmap_t *input, hp_bitmap_t *output) {
    hp_bitmap_init(output, input->width, input->height);
    memcpy(output->bitmap, input->bitmap, hp_bitmap_buffersize(output) );

    if(input->mask != NULL)
        memcpy(output->mask, input->mask, hp_bitmap_buffersize(output) );
    else
        output->mask = NULL;
}
...
void hp_bitmap_transpose(const hp_bitmap_t *input, hp_bitmap_t *output) {
    ...
    for(uint16_t y = 0; y<output->height; y++) { // Loop over each row of the output
        for(uint16_t b=0; b<bpr_out; b++) {      // Loop over each byte in the row
            ...
            output->bitmap[bpr_out*y+b]=0;
            output->mask[bpr_out*y+b]=0;
            ...
            for(uint8_t i=0; i<8; i++) {         // Loop over each pixel in the byte
                ...
                // Pixel in output (x,y) = maps to input(y,x)
                    if( (input->bitmap[hp_bitmap_pos_to_byte(input,  y, x)] & (0x01 << (y % 8)) ) !=0 )
                        output->bitmap[y*bpr_out+b] |= (0x01 << i);

                    if( (input->mask[hp_bitmap_pos_to_byte(input,  y, x)] & (0x01 << (y % 8)) ) !=0 )
                        output->mask[y*bpr_out+b] |= (0x01 << i);
                }
            }
        }
    }
...
void hp_bitmap_invert(hp_bitmap_t *bitmap) {
    for(uint16_t b=0; b<hp_bitmap_buffersize(bitmap); b++) bitmap->bitmap[b] = 0xFF - bitmap->bitmap[b];
}
...
void hp_bitmap_copy_from_array(hp_bitmap_t *bitmap, const uint8_t array[], const size_t array_size, const uint16_t width, const uint16_t height) {
    ...
    // Check if the size of array[] is sufficient
    if(array_size != hp_bitmap_buffersize(bitmap))
        ESP_LOGE(tag_buf, "Size %d of array does not match %dx%d bitmap", array_size, width, height);
    else
        memcpy(bitmap->bitmap, array, hp_bitmap_buffersize(bitmap) );
}
    ...
    for(uint16_t row = 0; row<bitmap->height; row++) {
        for(uint16_t b=0; b<bytes_per_row; b++) {
            for(uint8_t i=0; i<8; i++) {
                c[8*b+i] = ( ( (bitmap->bitmap[bytes_per_row*row+b]>>i) & 0x01) ==0 ) ? ' ' : 'O';
            }
        }
        printf("%s\n", c);
    }
