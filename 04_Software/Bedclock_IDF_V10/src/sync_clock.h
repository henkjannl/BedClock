...
static const char *tm_tag = "wifi station";
                                ...
                                int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        if (s_retry_num < ESP_MAXIMUM_RETRY) {
            esp_wifi_connect();
            s_retry_num++;
            ESP_LOGI(tm_tag, "retry to connect to the AP");
        } else {
            xEventGroupSetBits(s_wifi_event_group, WIFI_FAIL_BIT);
        }
        ESP_LOGI(tm_tag,"connect to the AP fail");
    } else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ip_event_got_ip_t* event = (ip_event_got_ip_t*) event_data;
        ESP_LOGI(tm_tag, "got ip:" IPSTR, IP2STR(&event->ip_info.ip));
        s_retry_num = 0;
...
void wifi_init_sta(void)
{
    ...
    ESP_LOGI(tm_tag, "wifi_init_sta finished.");
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ESP_LOGI(tm_tag, "connected to ap SSID:%s password:%s",
                 WIFI_SSID, WIFI_PASS);
    } else if (bits & WIFI_FAIL_BIT) {
        ESP_LOGI(tm_tag, "Failed to connect to SSID:%s, password:%s",
                 WIFI_SSID, WIFI_PASS);
    } else {
        ESP_LOGE(tm_tag, "UNEXPECTED EVENT");
    }
}
...
void time_init(void)
{
    ...
    ESP_LOGI(tm_tag, "ESP_WIFI_MODE_STA");
    wifi_init_sta();
